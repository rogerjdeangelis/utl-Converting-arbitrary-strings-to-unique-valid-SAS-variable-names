...
Recent Comments and solutio by Paul Dorfman
Paul Dorfman          
...
Of course, we only need to replace the *leftmost* hex digit in the response of the PUT function in case it is 0-9; elsewhere, the digits are fine. But what the heck, we can just replace all the digits in the result of PUT with letters it doesn't have in one fell swoop of the TRANSLATE action. Since the result of PUT contains only letters A-F, the uniqueness is preserved. 
ruth be told, this is a one-way approach: You can't take uniqueName generated this way and convert it back to AnyString; so, you will have to maintain a cross-reference table. However, this is as easy as storing the pairs in a character format.  
